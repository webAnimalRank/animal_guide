...
export default function BirthDay() {
  ...
  // 기능 (액션)
  const { fetchBirthVillagers, setCurrentPage, isItemToday, formatBirth } =
    useBirthStore((state) => state.actions);
  ...
  const displayedVillagers = birthVillagers.slice(
      ...
      <div className='relative'>
        <BirthWrap key={currentPage} className={isAllLoad ? 'load' : ''}>
          {displayedVillagers.map((item) => (
            <List
              key={item.villagerNo}
              className={isItemToday(item.villagerBirth) ? 'today' : ''}
            >
              <img
              ...
              />
              <span className='text-2xl max-md:text-xl font-extrabold'>
                {item.villagerName}
              </span>
              <span className='text-lg font-bold ml-auto'>
                {formatBirth(item.villagerBirth)}
              </span>
            </List>
          ))}
        </BirthWrap>
        {!isAllLoad && (
          <Loading className='absolute left-1/2 top-1/2 -translate-1/2  size-30' />
        )}
      </div>
