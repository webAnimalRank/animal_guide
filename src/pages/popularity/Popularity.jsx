		...
		<Wrap className='relative'>
			...
			<div className='flex-1 min-h-0 bg-(--c) rounded-2xl relative'>
				<SelectWrap>
					{char.map((v, i) => {
						...
						return (
							<SelectBox key={i}>
								<img src={v.img} alt='' className='min-h-0 flex-1 object-contain' />
								<div className='font-extrabold whitespace-nowrap name'>{v.name}</div>
								<input
									type='checkbox'
									name='check'
									className='hidden'
			...
			<div className='flex flex-0 gap-3 justify-between max-sm:flex-col sm:pt-2'>
				<CheckWrap>
					{checkId.map((i) => (
						<CheckBox key={i}>
							<img className='object-contain min-h-0 flex-1' src={char[i].mini} alt='' />
							{char[i].name}
