...
export const Txt = styled.div.attrs({
  className:
    'txt bg-(image:--glass) shadow-(--shadow) absolute inset-x-0 bottom-0 py-2 flex items-center justify-center font-bold text-xl opacity-0 text-white text-shadow-(--shadow2)'
})`
  @media (max-width: 480px) {
    font-size: 0.9rem;
...
export const CheckBox = styled.div.attrs({
  className:
    'size-full pb-2 flex flex-col justify-center bg-(image:--glass) shadow-(--shadow) rounded-2xl font-bold text-lg relative'
})`
  @media (max-width: 480px) {
    width: 100%;
    font-size: 0.9rem;
...
export const Submit = styled.button.attrs({
  type: 'button',
  className:
    'self-end h-max py-2 px-4 rounded-lg font-bold max-sm:w-full bg-(image:--glass2) shadow-(--shadow) text-white text-shadow-(--shadow2)'
})`
  background-color: yellowgreen;
  &:disabled {
    pointer-events: none;
