...
export const Mini = styled.button.attrs({
  type: 'button',
  className:
    'w-full p-1 font-extrabold border border-white/20 rounded-md text-white/60 hover:bg-(--p)/30'
})`
  img {
    width: 100%;
  }
...
export const List = styled.div.attrs({
  className:
    'text-base max-sm:text-sm flex gap-2 text-shadow-(--shadow) whitespace-nowrap'
})`
  &::before {
    content: '';
    width: 1rem;
...
export const Nav = styled.nav.attrs({
  className:
    'sticky top-23 max-sm:top-17 flex items-center justify-between z-10 px-1'
})``;
