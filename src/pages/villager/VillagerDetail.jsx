...
export default function VillagerDetail({ selectedNo, isOpen, onClose }) {
  ...
  const {
    ...
    loading,
    error
  } = useVillagerDetail(selectedNo, isOpen);
  ...
  return (
    <dialog ref={dialogRef} onClose={onClose}>
      <div
        className="fixed inset-0 bg-black/30 flex flex-col gap-8 items-center justify-center"
        onClick={onClose}
      >
        <Card onClick={(e) => e.stopPropagation()}>
          {loading && (
            <div className="p-10 text-center opacity-70">불러오는 중...</div>
          )}
          {error && (
            <div className="p-10 text-center text-red-500">에러 발생</div>
          )}
          {!loading && detail && (
            <>
