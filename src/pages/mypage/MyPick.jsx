	...
	return (
		<div className='flex flex-col gap-5 items-center'>
			...
			{pickedVillagers.length > 0 ? (
				...
				<div>{isCurrentMonth ? '아직 투표하지 않았습니다.' : '투표 내역이 없습니다.'}</div>
			)}
			...
			{/* 투표/결과 버튼 */}
			{!hasVotedThisMonth && isCurrentMonth && <Btn onClick={handleVoteClick}>투표하러 가기</Btn>}
			{/* 결과 확인 버튼은 항상 표시하고 싶으면 아래처럼 추가 가능 */}
			{hasVotedThisMonth && !isCurrentMonth && <Btn onClick={handleVoteClick}>결과 확인</Btn>}
		</div>
	);
}
